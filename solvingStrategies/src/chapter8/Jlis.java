...
public class Jlis { 
	...
	private int solve(int aIdx, int bIdx) {
		...
		long a, b, max;
		a = aIdx == -1 ? Long.MIN_VALUE : arrA.get(aIdx);
		b = bIdx == -1 ? Long.MIN_VALUE : arrB.get(bIdx);
		max = Math.max(a, b);
		for(int anIdx = aIdx+1; anIdx < arrA.size(); anIdx++) {
			if(max < arrA.get(anIdx)) 
				cache[aIdx+1][bIdx+1] = Math.max(cache[aIdx+1][bIdx+1], solve(anIdx, bIdx)+1);
		}
		for(int bnIdx = bIdx+1; bnIdx < arrB.size(); bnIdx++) {
			if(max < arrB.get(bnIdx)) 
				cache[aIdx+1][bIdx+1] = Math.max(cache[aIdx+1][bIdx+1], solve(aIdx, bnIdx)+1);
		}
